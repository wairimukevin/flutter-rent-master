...
class LandHouseDetail extends StatefulWidget {
  // Declare a field that holds the Todo.
  DocumentSnapshot snapshot;
  LandHouseDetail(this.snapshot);
  @override
  _LandHouseDetailState createState() => _LandHouseDetailState(snapshot);
}

class _LandHouseDetailState extends State<LandHouseDetail> {
DocumentSnapshot snapshot;   
  _LandHouseDetailState(this.snapshot);
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (await canLaunch(url)) {
                                          ...
                                          Fluttertoast.showToast(
                                            msg: "no tenet to call Phone.house not taken",
                                    ...
                                    child: IconButton(
                                      ...
                                      var url = "sms:"+snapshot.data()['mobileNo'].toString();
                                        if (await canLaunch(url)) {
                                          ...
                                          Fluttertoast.showToast(
                                            msg: "no tenant to sms.house not taken",
                                            toastLength: Toast.LENGTH_SHORT,
                                            gravity: ToastGravity.BOTTOM,
                                            timeInSecForIosWeb: 1,
                                            backgroundColor: Colors.black,
...
Widget getstatusCard(var context){
    var size = Screen(MediaQuery.of(context).size);
    return ownerSnapshot!=null ? Card(
      elevation: 0,
      borderOnForeground: true,
        child: InkWell(
          ...
          child: Container(
            ...
            child: Row(
              children: <Widget>[
                Container(
                  width: size.wp(100)-size.hp(10),
                  child: Padding(
                    padding: EdgeInsets.symmetric(vertical: size.getWidthPx(5),horizontal: size.getWidthPx(5)),
                    child: Column(
                      children: <Widget>[
                        Align(
                          alignment: Alignment.centerLeft,
                          child: snapshot.data().containsKey("tenantf") ?Text(
                            'tenant:${snapshot.data()['tenantf']}',
                            style: TextStyle(
                              fontFamily: 'Ex02',
                              fontSize: size.getWidthPx(20),
                              fontWeight: FontWeight.w500,
                          ...
                          ): Text("not taken",style: TextStyle(color: Colors.grey,fontSize: size.getWidthPx(16))),
                        ),
                        Row(
                          children: <Widget>[
                            snapshot.data().containsKey("tenante") ?
                            Text(
                              "tenant email:${snapshot.data()['tenante']},",
                              style: TextStyle(color: Colors.grey,fontSize: size.getWidthPx(16)),
                            ) : Text("not taken",style: TextStyle(color: Colors.grey,fontSize: size.getWidthPx(16))),
                          ],
                        ),
                        ...
                        Align(
                          alignment: Alignment.topLeft,
                          child:snapshot.data().containsKey("bookingdate") ? Text(
