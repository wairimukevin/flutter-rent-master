                              ...
                              child: Row(
                                ...
                                children: <Widget>[
                                  Expanded(
                                    child: TextFormField(
                                      decoration: new InputDecoration(
                                        hintText: 'Deposit Amount in KSH',
                                        fillColor: Colors.white,
                                        contentPadding:
                                            const EdgeInsets.all(10.0),
                                      ),
        ...
        children: <Widget>[
          Column(
            children: <Widget>[
              ...
              checkbox("males Only", boyVal),
              checkbox("female Only", girlVal),
...
/*void createRecord(context) async {
  ...
  ref = await databaseReference.collection("House")
    ...
    'Overview': overview,
    'favourite': 0,
    'houseImages': FieldValue.arrayUnion(imageDataPath),
    'builtUpArea': _buildup,
    'depositAmount': _deposit,
    'description': _detail,
    'monthlyRent': _monthly,
    'ownerDetail': userReference,
    'status': 'Available'
  });
  print(ref.id);
  Navigator.pop(context);
  ...
  Fluttertoast.showToast(msg: 'House Ad Posted Successfully');
   ...
   Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(builder: (context) => SearchResultPage(userRef)),
              (route) => false);
...
class _MultiImageState extends State<MultiImage> {
  ...
  List<File> imagePaths = List<File>();
  ...
  Future<Null> _addImages(var pic) async {
    var t = await FlutterAbsolutePath.getAbsolutePath(pic.identifier);
              ...
              : IconButton(
                ...
                onPressed: () async {
                  ...
                  .add({
                    ...
                    'description': _detail,
                    'monthlyRent': _monthly,
                    'ownerDetail':'/User/'+userReference,
                    'status': 'Available',
                    'firstName':fname,
                    'lastName':sname,
                    'email':email,
                  ...
                  var userRef= await LocalStorage.sharedInstance.loadUserRef(Constants.userRef);
                  Navigator.of(context).pushAndRemoveUntil(
                    MaterialPageRoute(builder: (context) => userhouse(userRef)),
                    (route) => false);
                },
              ),
           ] : null,
